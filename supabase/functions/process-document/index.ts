...
Deno.serve(async (req: Request) => {
  try {
    ...
    try {
      ...
      const rawDocId = job.payload?.document_id;
      if (!rawDocId || typeof rawDocId !== "string" || !UUID_REGEX.test(rawDocId)) {
        await supabaseClient
          .from("jobs")
          .update({ status: "failed", error_message: "Job payload contains invalid document_id UUID" })
          .eq("id", job.id);
        return jsonResponse(
          { error: "Invalid document_id UUID in job payload", details: { document_id: rawDocId } },
          400
        );
      }
      documentId = rawDocId;
    ...
    return jsonResponse(
      { error: "Fatal Edge Function exception", details: fatal?.message || String(fatal), stack: String(fatal?.stack || "") },
      500,
    );
  }
});