...
const DEMO_DOCUMENTS: Document[] = [
  { id: 'd1', patient_id: 'demo-1', filename: 'Lab Result Mar 2024.pdf', status: 'extracted', uploaded_at: '2024-03-14T10:00:00Z', storage_path: null },
  { id: 'd2', patient_id: 'demo-1', filename: 'Cardiology Visit Notes.pdf', status: 'extracted', uploaded_at: '2024-03-11T14:30:00Z', storage_path: null },
  { id: 'd3', patient_id: 'demo-1', filename: 'Lab Result Sep 2023.pdf', status: 'extracted', uploaded_at: '2023-09-05T09:15:00Z', storage_path: null },
]
...
Flag the Lisinopril prescription for review before the next appointment. Request a nephrology consult to assess trajectory. The rate of GFR decline (-18 points / 18 months) warrants specialist involvement.`,
  claims: [
    { id: 'c1', briefing_id: 'b1', claim_text: 'GFR has fallen from 65 to 47', source_type: 'document', source_id: 'd1', source_excerpt: 'eGFR 47 mL/min/1.73m²', source_page: 1, verification_status: 'supported', created_at: new Date().toISOString() },
    { id: 'c2', briefing_id: 'b1', claim_text: 'Lisinopril 10 mg daily', source_type: 'document', source_id: 'd2', source_excerpt: 'Lisinopril 10mg QD — new prescription', source_page: 2, verification_status: 'supported', created_at: new Date().toISOString() },
    { id: 'c3', briefing_id: 'b1', claim_text: 'contraindicated in the context of declining kidney function', source_type: 'drug_interaction', source_id: 'DDInter #4521', source_excerpt: 'ACE inhibitors contraindicated in eGFR < 30, caution below 60', source_page: null, verification_status: 'supported', created_at: new Date().toISOString() },
  ],
  flagged_concerns: [
    { severity: 'high', description: 'ACE inhibitor (Lisinopril) prescribed despite 18-month declining GFR trend — contraindicated in declining kidney function.' },
  ],
}
...
type FlaggedConcern = { severity: 'high' | 'medium' | 'low'; description: string }

// ─── Citation chip ────────────────────────────────────────────────────────────

function CitationChip({ claim, onDocClick }: { claim: Claim; onDocClick: (e: React.MouseEvent, id: string, page?: number) => void }) {
  const isDrug = claim.source_type === 'drug_interaction'
  const label = isDrug
    ? claim.source_id
    : `Doc · p.${claim.source_page ?? '?'}`

  return (
    <button
      onClick={(e) => !isDrug && onDocClick(e, claim.source_id, claim.source_page ?? undefined)}
...
function pipelineStep(status: string): number {
  ...
  if (status === 'extracted' || status === 'ready') return 2
  return 0
}
...
export default function PatientDetailClient({ patient, initialDocuments, initialBriefings }: Props) {
  ...
  const [audience, setAudience] = useState<'specialist' | 'gp' | 'family'>('specialist')
  ...
  const generateBriefing = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      const wrapperUrl = process.env.NEXT_PUBLIC_GRAPHITI_WRAPPER_URL
      if (!wrapperUrl) { alert('Briefing service not configured.'); return }
      const response = await fetch(`${wrapperUrl}/briefings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ patient_id: patient.id, audience, user_id: user.id }),
      })
      if (!response.ok) throw new Error('Briefing generation failed')
    } catch (err) {
      console.error('[v0] Briefing generation failed:', err)
  ...
  const retryBriefing = async (briefingId: string) => {
    ...
    try {
      ...
      await fetch(`${wrapperUrl}/briefings/${briefingId}/retry`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const supported = claimsArray.filter(c => c.verification_status === 'supported').length
  const partial = claimsArray.filter(c => c.verification_status === 'partial').length
  const unsupported = claimsArray.filter(c => c.verification_status === 'unsupported').length
            ...
            <div className="flex-1 space-y-1">
              {concerns.map((c, i) => (
                <div key={i} className="flex items-start gap-2">
                  ...
                  <p className="text-[12px] text-foreground leading-relaxed">{c.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {activeBriefing ? (
            <div className="px-8 py-6 max-w-3xl">
              ...
              <div className="flex items-start justify-between mb-5">
                <div>
                  <h2 className="text-[16px] font-semibold text-foreground">
                    {activeBriefing.audience === 'specialist' ? 'Specialist' :
                     activeBriefing.audience === 'gp' ? 'GP' : 'Family'} Briefing
                  </h2>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-mono text-[9px] px-2 py-1 rounded border ${
                    activeBriefing.status === 'completed' ? 'text-success border-success/30 bg-success-dim' :
                    activeBriefing.status === 'processing' ? 'text-accent border-accent/30 bg-accent-dim' :
                    activeBriefing.status === 'failed' ? 'text-alert border-alert/30 bg-alert-dim' :
              ...
              {/* Processing state */}
              {activeBriefing.status === 'processing' && (
                  ...
                  <ReactMarkdown
                    components={{
                      ...
                      p: ({ children }) => {
                        ...
                        const matched = claimsArray.filter(c => text.includes(c.claim_text))
                        return (
